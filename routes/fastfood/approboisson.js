...
const upload = multer();
import { Approboisson, Categorie, Ligneapproboisson, Mouvementstock, Produit, Uniteconversion, User } from '../../models/index.js';
...
router.post('/nouveau', async (req, res) => {
    ...
    try {
        ...
        const newAppro = await Approboisson.create({
        ...
        for (const ligne of lignes) {
            ...
            if (produit) {
                await produit.update({ stock: produit.stock + qteligne }, { transaction });
            }
        }
...
router.get('/liste', async (req, res) => {
    ...
    try {
        const { rows: appro, count: totalItems } = await Approboisson.findAndCountAll({
            order: [['id', 'DESC']],
...
router.get('/lignes/:appro_id', async (req, res) => {
    ...
    try {
        ...
        const lignes = await Ligneapproboisson.findAll({
            where: { appro_id },
            include: [
                {
...
router.put('/annuler/:id', async (req, res) => {
    try {
        ...
        const appro = await Approboisson.findByPk(id, {
            include: {
                model: Ligneapproboisson,
                include: {
        ...
        for (const ligne of appro.Ligneappros) {
            if (ligne.Uniteconversion) {
                await Uniteconversion.update(
                    { stock: ligne.Uniteconversion.stock - ligne.qteligne }, // Annulation = diminution du stock
                    { where: { id: ligne.produit_id } }
                );
                
                await Ligneapproboisson.update(
                    { statut: 'Annulé' }, 
                    { where: { id: ligne.id } }
                );
...
export { router as approboissonRouter }